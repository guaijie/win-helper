...
export default class Resource {
    ...
    static getProjectRootPath(editor: TextEditor) {
        if (workspace.workspaceFolders) {
            ...
            if (folderNum === 1) {
                ...
                for (let i = 0; i < folderNum; i++) {
                    ...
                    if (
                        regExp.test(
                            editor.document.uri.fsPath.split(sep).join("/")
                        )
                    ) {
        ...
        Resource.getPackageVersion(Resource.PROJECT_ROOT, "win-plus").then(
            version => {
                Resource.WIN_VERSION = version;
            }
        );
    }
        ...
        try {
            content = await Resource.get(
                join(
                    Resource.DOCS_PATH,
                    "docs",
                    Resource.WIN_VERSION || Resource.LOCAL_LATEST_DOC,
                    `${title}.md`
                )
            );
        } catch {
